...
def main(args):
    rid = args.raw_path.split(str="/")[-1]
    os.chdir('../public/results/' + rid + '/out')
    warn("Process raw scRNA-seq data")
    preprocessing(rid, args.raw_path)
    # runTangram(sc_adata, args)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--raw_path', metavar='<STRING>', default=None, help='the path of standard 10X data: matrix, '
    ...
    parser.add_argument('--cluster_label', metavar='<STRING>', default='leiden',
                        help='cluster label for mapping in cluster mode')
    parser.set_defaults(entry_point=main)
